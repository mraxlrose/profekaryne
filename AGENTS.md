...
O botão **Métricas** abre `admin/metrics.html`, um dashboard que usa a Google Analytics Data API via `/analytics` no Worker Cloudflare. A rota aceita apenas o token GitHub de colaboradoras com escrita no repositório. Para ativá-la, configurar no Cloudflare os segredos `GA4_PROPERTY_ID`, `GOOGLE_SERVICE_ACCOUNT_EMAIL` e `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY`; consultar `cloudflare-worker/README.md`. Nunca colocar a chave da conta de serviço no GitHub ou em `admin/`.

As listas editáveis do painel usam widgets próprios definidos em `admin/safe-list-widgets.js`: exclusões de planos, jogos, itens incluídos, parágrafos, serviços, horários, agendamentos e notas pedem confirmação e oferecem **Desfazer** enquanto o formulário está aberto. O botão **Descartar alterações** recarrega o formulário e descarta somente mudanças ainda não salvas; rascunhos já salvos permanecem intactos. Para alterações já publicadas, usar a restauração pelo Histórico.
